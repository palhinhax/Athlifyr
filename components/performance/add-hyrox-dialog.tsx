...
import type { HyroxCategory } from "./types";

interface AddHyroxDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess: () => void;
}

export function AddHyroxDialog({
  open,
  onOpenChange,
  onSuccess,
}: AddHyroxDialogProps) {
  ...
  const [category, setCategory] = useState<HyroxCategory>("OPEN_MEN");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: t("success"),
        description: t("hyrox.savedSuccess"),
      });
      ...
      console.error("Error saving HYROX entry:", error);
      toast({
        variant: "destructive",
        title: t("error"),
        description: t("hyrox.saveFailed"),
      });
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{t("hyrox.addTitle")}</DialogTitle>
        </DialogHeader>
