...
async function fuzzySearchVenueIds(search: string): Promise<string[]> {
  ...
  try {
    ...
    `;
    return fuzzyVenues.map((v) => v.id);
  } catch {
    console.log("pg_trgm extension not available, using simple ILIKE search");
    ...
    >`
      ...
      ORDER BY sort_order
    `;
    return simpleVenues.map((v) => v.id);
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      fuzzyVenueIds = await fuzzySearchVenueIds(search);
