...
import { COMMUNITY_PROMO_CONFIG } from "./config";
...
export default function CommunityPromoPage() {
  ...
  useEffect(() => {
    ...
    const videoTimings = COMMUNITY_PROMO_CONFIG.videos.map((video, index) => {
      const startTime = COMMUNITY_PROMO_CONFIG.videos
        .slice(0, index)
    ...
    const overlayTimeout = setTimeout(() => {
      setShowOverlay(true);
    }, COMMUNITY_PROMO_CONFIG.finalOverlay.delay);
  ...
  return (
    ...
    >
      {/* Video Layers */}
      {COMMUNITY_PROMO_CONFIG.videos.map((video, index) => (
        <motion.div
          key={video.src}
          ...
          transition={{
            duration: COMMUNITY_PROMO_CONFIG.animation.videoFadeDuration / 1000,
          }}
        >
          <video
            ref={(el) => {
      ...
      <AnimatePresence>
        {showOverlay && (
          <motion.div
            ...
            transition={{
              duration:
                COMMUNITY_PROMO_CONFIG.animation.textFadeInDuration / 1000,
            }}
          >
            <motion.div
            ...
            >
              ...
              >
                {COMMUNITY_PROMO_CONFIG.finalOverlay.tagline}
              </motion.h2>
              ...
              >
                {COMMUNITY_PROMO_CONFIG.finalOverlay.logo}
              </motion.h1>
              ...
              >
                {COMMUNITY_PROMO_CONFIG.finalOverlay.subtitle}
              </motion.p>
