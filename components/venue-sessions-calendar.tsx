...
export function VenueSessionsCalendar({
  ...
  // Calculate week range - memoized to prevent infinite re-renders
  ...
  // Stable ISO string for fetch dependency
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      ...
      params.set("from", weekStartISO);
      params.set("to", weekEndISO);
  ...
  }, [venueId, weekStartISO, weekEndISO, userId, t, toast]);
      ...
      {weekDays.map((day) => {
        ...
        return (
          ...
          >
            <div
              className={cn(
                "mb-2 flex items-center justify-center gap-1",
            ...
            >
              ...
              {isOwnerOrAdmin && (
                <Button
                  ...
                  size="icon"
                  className="h-6 w-6"
                  onClick={() => openCreateSessionModal(day)}
  ...
  const renderListView = () => {
    const allSessions = weekDays.flatMap((day) => {
      const daySessions = getSessionsForDay(day);
      return daySessions.map((session) => ({ ...session, day }));
    });
    ...
    // Group by day
    const groupedByDay: Record<string, VenueSession[]> = {};
    allSessions.forEach(({ day, ...session }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      ) : sessions.length === 0 && viewMode === "week" ? (
        <div className="rounded-lg border border-dashed p-12 text-center">
          <Calendar className="mx-auto mb-4 h-12 w-12 text-muted-foreground/50" />
          <h3 className="text-lg font-medium">{t("noSessionsThisWeek")}</h3>
          <p className="mt-1 text-sm text-muted-foreground">
