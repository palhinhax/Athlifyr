...
import { CROSSFIT_PROMO_CONFIG } from "./config";

export default function CrossFitPromoPage() {
  ...
  useEffect(() => {
    ...
    const videoTimings = CROSSFIT_PROMO_CONFIG.videos.map((video, index) => {
      const startTime = CROSSFIT_PROMO_CONFIG.videos
        .slice(0, index)
        .reduce((acc, v) => acc + v.duration, 0);

      return setTimeout(() => {
        if (index < CROSSFIT_PROMO_CONFIG.videos.length - 1) {
    ...
    const overlayTimeout = setTimeout(() => {
      setShowOverlay(true);
    }, CROSSFIT_PROMO_CONFIG.finalOverlay.delay);
  ...
  return (
    ...
    >
      {/* Video Layers */}
      {CROSSFIT_PROMO_CONFIG.videos.map((video, index) => (
        <motion.div
          key={video.src}
          ...
          transition={{
            duration: CROSSFIT_PROMO_CONFIG.animation.videoFadeDuration / 1000,
          }}
        >
          <video
            ref={(el) => {
      ...
      <AnimatePresence>
        {showOverlay && (
          <motion.div
            ...
            transition={{
              duration:
                CROSSFIT_PROMO_CONFIG.animation.textFadeInDuration / 1000,
            }}
          >
            <motion.div
            ...
            >
              ...
              >
                {CROSSFIT_PROMO_CONFIG.finalOverlay.tagline}
              </motion.h2>
              ...
              >
                {CROSSFIT_PROMO_CONFIG.finalOverlay.logo}
              </motion.h1>
              ...
              >
                {CROSSFIT_PROMO_CONFIG.finalOverlay.subtitle}
              </motion.p>
