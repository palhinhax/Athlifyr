...
| `PrivacyMode` enum (PUBLIC/FRIENDS/ORGANIZER) | `LiveRaceVisibility` enum on User model | User-level setting; enforced in leaderboard/positions filtering |
...
| `PrivacyMode` (FRIENDS)                         | ✅ Implemented — Athlifyr has a full friendship system (`Friendship` model, API, UI); `LiveRaceVisibility` enum (`PUBLIC`/`FRIENDS`/`ORGANIZER_ONLY`) is enforced in live server |
...
| `PrivacyMode`   | ✅ Created  | `LiveRaceVisibility` enum on User model (`PUBLIC`/`FRIENDS`/`ORGANIZER_ONLY`) |
...
| Privacy mode respected in track and leaderboard           | ✅     | `LiveRaceVisibility` enforced: PUBLIC=all, FRIENDS=friends+self+organizer, ORGANIZER_ONLY=organizer+self. Leaderboard anonymizes hidden entries; positions exclude hidden athletes. |
...
5. **~~Privacy mode~~** — ✅ Implemented. `LiveRaceVisibility` enum (`PUBLIC` / `FRIENDS` / `ORGANIZER_ONLY`) per user, enforced in live server leaderboard/position filtering. User setting in Account Settings → Privacy tab.
...
The one item from the original plan that is **not implemented** is:

1. **Persistent tracking points** (`TrackingPoint` model) — replaced by volatile
...
Privacy mode (`PUBLIC` / `FRIENDS` / `ORGANIZER_ONLY`) is now **fully implemented**:

- `LiveRaceVisibility` enum on the User model (default: `PUBLIC`)
- User setting accessible in Account Settings → Privacy tab
- Live server enforces visibility in leaderboard and position broadcasts
- Leverages Athlifyr's existing friendship system (`Friendship` model, `/api/friends/` endpoints)
